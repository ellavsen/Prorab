...
# Как процент называется человеку. Живёт рядом с самим основанием, потому что
# читателей у этих слов четверо — бот, PDF, публичная страница и XLSX, — и
# четыре копии разошлись бы так же, как расходились четыре вычислителя до
# Sprint 1. Тот же довод, по которому STATUS_LABEL лежит рядом с EstimateStatus.
#
# Основание пишется словами всегда, в обоих вариантах: двусмысленно в «6%»
# именно оно, а не слово перед процентом. Формулировка для заказчика выбрана из
# трёх и обоснована в ADR-024 §5 — «по договору» верна при любом договоре, тогда
# как «налог» утверждает причину, которая у следующего заказчика может быть иной.
# Словарь, а не готовые предложения: у чата, документа и таблицы вёрстка разная
# по делу, а слова обязаны совпадать. Поверхность собирает свою строку сама, но
# из этих двух словарей.
RATE_OF = {RateBase.COST: "к цене", RateBase.PRICE: "от суммы"}
MARKUP_WORD = {RateBase.COST: "наценка", RateBase.PRICE: "по договору"}


def rate_caption(rate: Decimal, base: RateBase = RateBase.COST) -> str:
    """«наценка 6,00% к цене» / «по договору 6,00% от суммы»."""
    kind = RateBase(base)
    return f"{MARKUP_WORD[kind]} {format_money(rate)}% {RATE_OF[kind]}"


def rate_note(
    markup: Decimal, subtotal: Decimal, base: RateBase = RateBase.COST
) -> str:
    """Расшифровка итога документа.

    Надбавка — разность, и проверить её умножением документ не предлагает: на
    смете это равенство не выполняется (money.md §3.5).
    """
    return (
        f"в том числе {MARKUP_WORD[RateBase(base)]} {format_money(markup)} ₽ "
        f"(без надбавки {format_money(subtotal)} ₽)"
    )
