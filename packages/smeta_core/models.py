...
class Category(StrEnum):
    """Значения английские: это код, а не экран (конституция про язык).

    Русские подписи живут в адаптерах — в текстах бота и заголовках листов
    Excel. До Sprint 5 здесь лежали «Работа»/«Материал», и они же попадали в
    базу и в HTTP-контракт; миграция переводит старые значения.
...
class PositionData:
    ...
    unit — каноническая единица из справочника, она для аналитики.
    unit_spoken — как единицу назвал человек («мешков»); именно она печатается
    в документе заказчику. В арифметике не участвует ни та, ни другая
    (ADR-015).
    """
