...
def display_unit(unit: str, unit_spoken: str) -> str:
    """Единица там, где она стоит сама по себе: «₽/мешок», колонка «Ед.».

    Сказанное приводится к словарной форме («мешков» → «мешок»), но каноном
    НЕ подменяется: ADR-015 требует, чтобы заказчик узнавал свои слова, а
    падеж — не другая единица. Слово вне закрытого словаря тары остаётся как
    сказано: угадывать формы морфологией мы не беремся.

    Для фразы «20 мешков» не годится и там не применяется: количество
    согласовано с падежом, и словарная форма сломала бы русский.
    """
    return packaging_form(unit_spoken) or unit_spoken or unit
